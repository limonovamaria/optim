"""Модуль содержит методы формирования целевых функций (включая штрафные)"""
...
def create_func_2(k_targ, ka, kb, kc, kd, a_min, a_max, b_min, b_max, c_min, c_max, d_min, d_max, k=100, p=2):
    def inner_method(x):
        ...
        for xx in [xa, xb, xc, xd]:
            xx_new = sorted(xx)
            f_res += k * max(-(xx_new[-1] - (xx_new[-2] * 10**2)), 0) ** p
